...
def poisson_page():
    st.title("Poisson equation benchmark", anchor=False)

    st.markdown("This benchmark reapresents a linear elliptic PDE, given by:")
    st.latex(
        r"""
        \begin{cases}
            -\Delta u = f \quad & in \ D         \\
    ...
    st.markdown(
        ...
        are the evaluation of the solution operator $\mathcal{G}: f \\to u$ 
        from the source term to the solution."""
    )
    ...
    ## Read the data
    data = read_data("poisson", model, hyperparams_mode.replace("_", ""))
