...
def allen_page():
    st.title("Allen-Cahn equation benchmark", anchor=False)

    st.markdown("This benchmark represents a non-linear PDE, given by:")
    st.latex(
        r"""\begin{cases}
                  \frac{\partial u}{\partial t} = \Delta u - \varepsilon^2 u (u^2-1) \quad & in \ D\times (0, T) \\
                  u(x, 0) = f(x)                                                           & in\ D               \\
                  \varepsilon = 220
              \end{cases}"""
    )
    st.markdown(
        ...
        that maps the initial condition $f$ into the solution at the final time $T=0.0002$.
        """
    )
